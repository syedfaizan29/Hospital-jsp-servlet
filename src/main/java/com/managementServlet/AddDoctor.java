...
public class AddDoctor extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        try {
            ...
            if (f) {
                System.out.println("[managementServlet.AddDoctor.java]-- New record added of Doctor details in doctor_details database successfully bt management login page.");
                ...
                System.out.println("[managementServlet.AddDoctor.java]-- Something went wrong on server to add new Doctor, please do correct.");
                session.setAttribute("errorMsg", "Something went wrong on server please try after sometime.");
                resp.sendRedirect("management/doctorPage.jsp");
            }
        } catch (Exception e) {
            System.out.println("[managementServlet.AddDoctor.java]-- An error/exception has been found while running the method protected void doPost(){} " + e);
            HttpSession session = req.getSession();
